...
def test_audit_dashboard_contains_badges(badge):
    ...
    repo_root = Path(__file__).parent.parent.parent
    with open(
        repo_root / "docs" / "audit" / "dashboard.md", "r", encoding="utf-8"
    ) as f:
        content = f.read()
...
@pytest.mark.parametrize(
    "element",
    ["name: Audit Dashboard Update", "cron: '25 8 * * 1'", "make audit-dashboard"],
)
def test_workflow_file_exists(element):
    """Test that the audit dashboard cron workflow exists."""
    repo_root = Path(__file__).parent.parent.parent
    with open(
        repo_root / ".github" / "workflows" / "audit-dashboard-cron.yml",
        "r",
        encoding="utf-8",
    ) as f:
        content = f.read()
...
@pytest.mark.parametrize(
    "link",
    ["dependency_inventory.csv", "vulnerability_report.csv", "license_report.csv"],
)
