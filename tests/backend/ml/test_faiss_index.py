...
class TestFAISSIndex:
    ...
    def test_hnsw_index_creation(self):
        ...
        assert index.config["ef_construction"] == 200
        assert index.config["ef_search"] == 64

    def test_opq_hnsw_index_creation(self):
        """Test OPQ+HNSW index creation."""
        index = FAISSIndex(dimension=64, index_type="OPQ+HNSW")
        assert index.index is not None
        assert index.index.d == 64
        # Check default config
        assert index.config["m_pq"] == 8
        assert index.config["n_subquantizers"] == 32
        assert index.config["m_hnsw"] == 16
    ...
    def test_custom_config(self):
        ...
        index = FAISSIndex(dimension=64, index_type="HNSW", config=custom_config)
        assert index.config == custom_config
    ...
    def test_save_load_hnsw_index(self):
        """Test saving and loading HNSW index with config."""
        index = FAISSIndex(dimension=16, index_type="HNSW")
        ...
        # Save index
        with tempfile.TemporaryDirectory() as tmpdir:
            save_path = os.path.join(tmpdir, "hnsw_index")
            index.save_index(save_path)

            # Create new index and load
            new_index = FAISSIndex(dimension=16)
            new_index.load_index(save_path)

            # Verify configuration was preserved
            assert new_index.index_type == "HNSW"
            assert new_index.config == index.config
            assert new_index.device == index.device
    ...
    def test_from_tuning_result(self):
        """Test creating index from tuning result."""
        tuning_result = {
            "index_type": "HNSW",
            "parameters": {
                "m": 16,
        ...
        index = FAISSIndex.from_tuning_result(dimension=64, tuning_result=tuning_result)
        assert index.dimension == 64
        assert index.index_type == "HNSW"
        assert index.config == tuning_result["parameters"]

    def test_opq_hnsw_training(self):
        """Test OPQ+HNSW training process."""
        index = FAISSIndex(dimension=16, index_type="OPQ+HNSW")

        # Generate training data
        embeddings = np.random.randn(100, 16).astype(np.float32)
        ids = [f"id{i}" for i in range(100)]

        # Add embeddings (should trigger OPQ training)
        index.add_embeddings(embeddings, ids)

        # Check that OPQ is trained
        assert index._opq_trained
        assert index.index.ntotal == 100

    def test_hnsw_dynamic_search_parameters(self):
        """Test HNSW dynamic search parameter updates."""
        index = FAISSIndex(dimension=16, index_type="HNSW")
        ...
        # Search should set efSearch dynamically
        query = np.random.randn(16).astype(np.float32)
        results = index.search(query, k=5)

        # Verify efSearch was set from config
        assert index.index.hnsw.efSearch == index.config["ef_search"]
...
class TestAlertSearchEngine:
    ...
    def test_initialization(self, engine):
        ...
        assert engine.index.index_type == "Flat"  # This test uses Flat explicitly

    def test_default_initialization(self, mock_embedder):
        """Test default engine initialization (should use HNSW)."""
        engine = AlertSearchEngine(embedder=mock_embedder)
        assert engine.index.index_type == "HNSW"  # Now defaults to HNSW
