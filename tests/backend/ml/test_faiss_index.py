...
from backend.alfred.ml.faiss_index import FAISSIndex, SearchResult, AlertSearchEngine
...
class TestFAISSIndex:
    ...
    def test_add_embeddings_with_metadata(self, index, sample_embeddings, sample_ids):
        ...
        metadata = [{"severity": "high"}, {"severity": "low"}, {"severity": "medium"}, 
...
class TestAlertSearchEngine:
    ...
    def test_index_alerts(self, engine, mock_embedder):
        ...
        alerts = [
            ...
            {"id": "3", "name": "Disk Full", "description": "No space left", "severity": "critical"}
        ]
    ...
    def test_search_similar(self, engine, mock_embedder):
        ...
        mock_embedder.embed.return_value = np.array([
            [1.0, 0.0, 0.0],  # First alert
            [0.0, 1.0, 0.0]   # Second alert  
        ]).astype(np.float32)
