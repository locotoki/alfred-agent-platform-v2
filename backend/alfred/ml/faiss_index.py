...
class FAISSIndex(Service):
    ...
    def batch_search(self,
        ...
        if query_embeddings.shape[1] != self.dimension:
            raise ValueError(f"Expected dimension {self.dimension}, got {query_embeddings.shape[1]}")
...
class AlertSearchEngine:
    ...
    def index_alerts(self, alerts: List[Dict]):
        ...
        for alert in alerts:
            text = f"{alert.get('name', '')} {alert.get('description', '')} {alert.get('summary', '')}"
            texts.append(text)
...
if __name__ == "__main__":
    ...
    if args.action == "build":
        ...
        sample_alerts = [
            {"id": "1", "name": "Database Error", "description": "Connection timeout", "severity": "critical"},
            {"id": "2", "name": "API Latency", "description": "High response time", "severity": "warning"},
            {"id": "3", "name": "Memory Usage", "description": "High memory consumption", "severity": "warning"},
        ]
