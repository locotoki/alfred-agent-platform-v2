...
class FAISSIndex(Service):
    ...
    def __init__(
        ...
        Args:
            ...
            index_type: Index type (IVF, LSH, HNSW, OPQ+HNSW, Flat)
            nlist: Number of clusters for IVF
            nprobe: Number of clusters to search
            device: Device to use (cpu/gpu)
            config: Additional configuration for advanced index types
        """
        ...
        # Use optimized configurations for HNSW and OPQ+HNSW
        if config is None:
            if index_type == "HNSW":
                # Optimized HNSW configuration from tuning
                self.config = {
                    "m": 32,
                    "ef_construction": 200,
                    "ef_search": 64,
                }
            elif index_type == "OPQ+HNSW":
                # Optimized OPQ+HNSW configuration from tuning
                self.config = {
                    "m_pq": 8,
                    "n_subquantizers": 32,
                    "m_hnsw": 16,
        ...
        # For OPQ+HNSW, track if OPQ is trained
        self._opq_trained = False
    ...
    def _create_index(self) -> faiss.Index:
        ...
        if self.index_type == "Flat":
            ...
            ef_search = self.config.get("ef_search", 64)

            index = faiss.IndexHNSWFlat(self.dimension, m)
            index.hnsw.efConstruction = ef_construction
            index.hnsw.efSearch = ef_search

        elif self.index_type == "OPQ+HNSW":
            # Optimized Product Quantization + HNSW
            m_pq = self.config.get("m_pq", 8)
            n_subquantizers = self.config.get("n_subquantizers", 32)
            m_hnsw = self.config.get("m_hnsw", 16)
            ef_construction = self.config.get("ef_construction", 200)
            ef_search = self.config.get("ef_search", 64)
            ...
            hnsw_index = faiss.IndexHNSWFlat(self.dimension, m_hnsw)
            hnsw_index.hnsw.efConstruction = ef_construction
            hnsw_index.hnsw.efSearch = ef_search

            # Combine: OPQ -> PQ with HNSW search
            index = faiss.IndexPreTransform(opq, pq_index)

            # Store OPQ for training
            self._opq = opq
    ...
    def add_embeddings(
        ...
        # Train index if needed (for IVF and OPQ)
        ...
        elif self.index_type == "OPQ+HNSW" and not self._opq_trained:
            start_time = time.time()
            # Train OPQ transformation
            if hasattr(self, "_opq"):
                self._opq.train(embeddings)
                self._opq_trained = True
                train_time = time.time() - start_time
                print(f"OPQ trained in {train_time:.3f}s")
    ...
    def search(
        ...
        if self.index_type == "IVF":
            ...
            # Set search effort dynamically
            ef_search = self.config.get("ef_search", 64)
            self.index.hnsw.efSearch = ef_search
        elif self.index_type == "OPQ+HNSW":
            # No direct search parameter setting needed for OPQ+HNSW
            pass
    ...
    def save_index(self, path: str):
        ...
        metadata = {
            ...
            "device": self.device,
            "config": self.config,
            "_opq_trained": getattr(self, "_opq_trained", False),
        }
    ...
    def load_index(self, path: str):
        ...
        self._opq_trained = metadata.get("_opq_trained", False)
    ...
    @classmethod
    def from_tuning_result(cls, dimension: int, tuning_result: Dict) -> "FAISSIndex":
        """Create index from tuning result.

        Args:
            dimension: Vector dimension
            tuning_result: Result from FAISSTuner

        Returns:
            Configured FAISSIndex instance
        """
        index_type = tuning_result["index_type"]
        config = tuning_result["parameters"]
...
class AlertSearchEngine:
    ...
    def __init__(
        self, embedder: Optional[HFEmbedder] = None, index_type: str = "HNSW", device: str = "cpu"
    ):
        """Initialize search engine.

        Args:
            embedder: HF embedder instance
            index_type: FAISS index type (defaults to optimized HNSW)
            device: Device to use
        """
        self.embedder = embedder or HFEmbedder(device=device)

        # Default to HNSW for best performance
