...
class RetrainPipeline(Service):
    ...
    def train_model(self, 
                   ...
                   test_size: float = 0.2,
                   random_state: int = 42) -> Tuple[RandomForestClassifier, Dict[str, float]]:
    ...
    def promote_model(self, run_id: str, metrics: Dict[str, float]) -> bool:
        ...
        if (metrics["f1_score"] >= self.min_f1_score and 
            metrics["precision"] >= self.min_precision and
            metrics["false_negative_rate"] <= 0.02):
    ...
    def run_pipeline(self, 
                    dataset_source: str,
                    force_promotion: bool = False) -> Dict:
