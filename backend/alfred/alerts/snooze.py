...
class AlertSnoozeService(SnoozeService):
    ...
    async def snooze_alert(self,
                          ...
                          user_id: Optional[str] = None) -> AlertSnooze:
        ...
        """
        # Validate duration
        duration = max(self.config.min_duration, 
                      min(duration, self.config.max_duration))
        ...
        # Emit metrics
        if self.metrics:
            self.metrics.increment('alert_snoozes_created', {
    ...
    async def unsnooze_alert(self,
        ...
        # Emit metrics
        if self.metrics:
            self.metrics.increment('alert_snoozes_unsnoozed', {
                'manual': True
            })
    ...
    async def check_alert_changed(self, 
                                 alert: AlertProtocol) -> bool:
    ...
    async def auto_unsnooze_if_changed(self,
                                      alert: AlertProtocol) -> bool:
        ...
        if await self.check_alert_changed(alert):
            return await self.unsnooze_alert(
                alert.id,
                reason="Alert properties changed",
                user_id="system"
            )
        return False
    
    async def extend_snooze(self,
                           alert_id: str,
                           additional_duration: int,
                           user_id: Optional[str] = None) -> Optional[AlertSnooze]:
    ...
    async def get_snooze_history(self,
                               alert_id: str,
    ...
    async def _create_audit_entry(self,
        ...
        self.redis.setex(
            audit_key,
            86400 * self.config.audit_retention_days,
