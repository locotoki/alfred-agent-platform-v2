        ...
        for service, service_config in config.get('services', {}).items():
            ...
            for rule_config in service_config.get('rules', []):
                ...
                rule = GroupingRule(
                    ...
                    similarity_threshold=rule_config.get('similarity_threshold', self.default_similarity_threshold),
        ...
        for service, service_rules in self.rules.items():
            ...
            for rule in service_rules:
                ...
                for condition in rule.conditions:
                    valid_ops = ['eq', '==', 'ne', '!=', 'gt', '>', 'gte', '>=', 
                               'lt', '<', 'lte', '<=', 'in', 'not_in', 'contains', 'matches']
                    if condition.operator not in valid_ops:
                        service_errors.append(f"Invalid operator in rule {rule.name}: {condition.operator}")
