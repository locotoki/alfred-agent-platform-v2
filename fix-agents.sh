...
class FinancialTaxAgent:
    def __init__(self, pubsub_transport, supabase_transport, policy_middleware):
        ...
        self.supported_intents = ["TAX_CALCULATION", "FINANCIAL_ANALYSIS", "TAX_COMPLIANCE_CHECK"]
...
# Create model registry schema and tables
echo "Initializing model registry database schema..."
docker exec db-postgres sh -c "psql -U postgres -d postgres -f /docker-entrypoint-initdb.d/model-registry-init.sql"

echo "Restarting services..."
docker-compose -f docker-compose.unified.yml restart model-registry agent-financial agent-legal
