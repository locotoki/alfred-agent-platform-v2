...
async def send_message_async(message: str) -> str:
    ...
    try:
        ...
        if hasattr(st.session_state, "use_direct_inference") and st.session_state.use_direct_inference:
            ...
            if not hasattr(st.session_state, "model_router_client"):
                st.session_state.model_router_client = create_model_router_client(st.session_state.model_router_url)

            # Use model router client for direct model inference
            model_id = st.session_state.selected_model

            if st.session_state.debug_mode:
                st.sidebar.write("Sending to Model Router", {
                    "message": message,
                    "model": model_id
                })
...
def process_command(message: str) -> str:
    ...
    if cmd == "help":
        ...
        # Still send to backend but with nicer formatting
        response = send_message(command)
        if "This is a simulated response" in response:
            return "Pong! Alfred is responsive (simulated response)."
        return response
    elif cmd == "models":
        # Create a response with model information
        return get_models_response()
    elif cmd == "trend" and args:
        # Format trend command before sending
        return send_message(f"trend {args}")
    else:
...
def sidebar_config():
    ...
    if st.session_state.use_direct_inference:
        model_router_url = st.sidebar.text_input(
            "Model Router URL",
            value=st.session_state.model_router_url
        )
                ...
                if st.session_state.use_direct_inference:
                    try:
                        ...
                        if router_response.status_code == 200:
                            st.success(f"Connected to Model Router ({router_response.elapsed.total_seconds():.3f}s)")
                        else:
                            st.warning(f"Model Router health check failed: {router_response.status_code}")
                ...
                if st.session_state.use_direct_inference:
                    try:
                        ...
                        if ollama_response.status_code == 200:
                            ...
                            st.success(f"Connected to Ollama - Available models: {', '.join(model_names)}")
                        else:
...
def send_message_sync(message):
    ...
    try:
        ...
        elif any(word in message.lower() for word in ["who", "what", "about you", "yourself", "tell me about"]):
        ...
        elif any(word in message.lower() for word in ["can", "able", "capability", "do for me", "help me"]):
