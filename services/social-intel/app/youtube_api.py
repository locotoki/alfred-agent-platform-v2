...
async def search_videos(
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params) as response:
                if response.status != 200:
                    ...
                    logger.error("youtube_api_search_error", 
        ...
        logger.info("youtube_api_search_success", 
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params) as response:
                if response.status != 200:
                    error_text = await response.text()
                    logger.error("youtube_api_video_details_error", 
                                status=response.status, 
                                error=error_text)
                    raise YouTubeAPIError(f"YouTube API error: {error_text}")
        ...
        logger.info("youtube_api_video_details_success", 
                   items_count=len(data.get("items", [])))
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params) as response:
                if response.status != 200:
                    error_text = await response.text()
                    logger.error("youtube_api_channel_details_error", 
                                status=response.status, 
                                error=error_text)
                    raise YouTubeAPIError(f"YouTube API error: {error_text}")
        ...
        logger.info("youtube_api_channel_details_success", 
                   items_count=len(data.get("items", [])))
...
async def get_trends_by_category(
    ...
    subcategory_queries = {
        "kids.nursery": ["nursery rhymes", "baby songs", "toddler music", "kids learning songs", 
                         "alphabet songs", "counting songs", "lullaby for babies"],
        ...
        "education.courses": ["online course", "skill tutorial", "programming tutorial", "learning guide"]
    }
    ...
    for video in niche_data[:20]:  # Analyze top 20 videos
        ...
        for i in range(len(title_words) - 1):
            ...
            if len(phrase) > 5 and phrase not in ["official video", "official music", "music video"]:
                key_phrases.append(phrase)
            
            if i < len(title_words) - 2:
                phrase = " ".join(title_words[i:i+3])
                if len(phrase) > 8 and phrase not in ["official music video", "official video lyrics"]:
                    key_phrases.append(phrase)
        ...
        for phrase in key_phrases:
            if phrase not in keywords and len(phrase) > 5:
                ...
                if len(related_videos) >= 2:
                    ...
                    avg_engagement = sum(v["engagement_ratio"] for v in related_videos) / len(related_videos)
                    ...
                    niches.append({
                        ...
                        "top_channels": [{"name": v["channel_name"], "subs": v["subscriber_count"]} 
    ...
    analysis = {
        ...
        "most_shorts_friendly": next((n["query"] for n in niches if n.get("shorts_friendly", False)), 
                                   niches[0]["query"] if niches else ""),
        "lowest_competition": next((n["query"] for n in niches if n.get("competition_level") == "Medium"), 
                                 niches[0]["query"] if niches else "")
    }
    ...
    if niches:
        ...
        recommendations.append("Target trending topics with high search volume but moderate competition")
