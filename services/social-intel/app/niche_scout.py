        ...
        with LatencyTimer(SI_LATENCY_SECONDS, {"endpoint": "/niche-scout"}):
            try:
                ...
                if db_niches and len(db_niches) > 0:
                    ...
                    self.logger.info(
                        "Successfully processed niches from database",
                    ...
                    self.logger.warning(
                        "No niches found in database, falling back to other sources"
                    )
                    ...
                    if not youtube_api_key:
                        ...
                        try:
                            ...
                            self.logger.info(
                                "Using YouTube API to get trends",
                            ...
                            for i, niche in enumerate(api_results.get("trending_niches", [])):
                                try:
                                    ...
                                    position_factor = 1.0 - (
                                        i / len(api_results.get("trending_niches", []))
                                    )
                                    ...
                                    self.logger.error(
                                        "Failed to save niche to database",
                                        niche=niche.get("query"),
                                        error=str(e),
                                    )
        ...
        return {
            ...
            "analysis_summary": {
                ...
                "most_shorts_friendly": next(
                    (n["name"] for n in niches if n.get("shorts_friendly", False)),
                    niches[0]["name"],
                ),
