...
os.environ["DATABASE_URL"] = os.environ.get("DATABASE_URL", "postgresql://postgres:your-super-secret-password@localhost:5432/postgres")
...
async def test_insert_feature(test_db):
    ...
    row = await test_db.fetchrow(
        "SELECT * FROM features WHERE phrase = $1",
        test_phrase
    )
...
async def test_update_feature_scores(test_db):
    ...
    row = await test_db.fetchrow(
        "SELECT * FROM features WHERE niche_id = $1",
        niche_id
    )
