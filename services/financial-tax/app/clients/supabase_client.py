"""
Enhanced Supabase client for financial-tax service.
...
# Define table names
FINANCIAL_IN_TABLE = "financial_in"
FINANCIAL_OUT_TABLE = "financial_out"


class SupabaseClient:
    """
    Enhanced Supabase client for the financial-tax service.
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                response = await client.get(
                    f"{SUPABASE_URL}/rest/v1/{FINANCIAL_OUT_TABLE}",
                    headers={
    ...
    async def store_task(envelope: Dict[str, Any]) -> str:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.post(
                    f"{SUPABASE_URL}/rest/v1/{FINANCIAL_IN_TABLE}",
                    headers={
                        "apikey": SUPABASE_SERVICE_ROLE_KEY,
                        "Authorization": f"Bearer {SUPABASE_SERVICE_ROLE_KEY}",
                        "Content-Type": "application/json",
                ...
                if response.status_code >= 400:
                    ...
                    if response.status_code == 404:
                        logger.error(f"Table '{FINANCIAL_IN_TABLE}' may not exist. Run setup script.")
                else:
    ...
    @staticmethod
    async def store_result(task_id: str, result: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code >= 400:
                    ...
                    if response.status_code == 404:
                        logger.error(f"Table '{FINANCIAL_OUT_TABLE}' may not exist. Run setup script.")
                    return False
    ...
    @staticmethod
    async def list_tasks(limit: int = 50, offset: int = 0, status: Optional[str] = None) -> List[Dict[str, Any]]:
        """
