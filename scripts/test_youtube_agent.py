...
async def test_blueprint(seed_url=None, auto_niche=False):
    ...
    print(f"Running Blueprint with seed URL: {seed_url if seed_url else 'Auto-selected from niches'}")
    
    result = await youtube_blueprint_flow(
        seed_url=seed_url,
...
def parse_args():
    ...
    parser.add_argument(
        "--queries",
        nargs="+",
        help="Queries for Niche Scout"
    )
    
    parser.add_argument(
        "--seed-url",
        help="Seed URL for Blueprint"
    )
    
    parser.add_argument(
        "--auto-niche",
        action="store_true",
        help="Auto-select niche for Blueprint"
    )
