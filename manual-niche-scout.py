    ...
    if len(niches) >= 3:
        ...
        guide = {
            ...
            "content_strategy": [
                ...
                "Include these keywords in titles: " + ", ".join([k for k in identify_video_pattern([v for n in niches[:3] for v in n.get('sample_videos', [])])['common_title_phrases'][:3]])
            ]
        }
...
async def analyze_niche(category, subcategory):
    ...
    # Get channel details
    channels = await get_channel_details(channel_ids[:30], youtube=youtube)  # Increased from 25 to 30
    ...
    for keyword, count in top_keywords:
        ...
        matching_videos = [v for v in all_videos if keyword.lower() in v["title"].lower() or 
                           keyword.lower() in " ".join(v.get("tags", [])).lower()]
        ...
        if len(matching_videos) >= 2:
            ...
            avg_likes = sum(v["like_count"] for v in matching_videos) / len(matching_videos) if any(v["like_count"] for v in matching_videos) else 0
            avg_comments = sum(v["comment_count"] for v in matching_videos) / len(matching_videos) if any(v["comment_count"] for v in matching_videos) else 0
            engagement = (avg_likes / avg_views * 100) if avg_views > 0 else 0
            
            # Calculate freshness (recency)
            avg_freshness = sum(v.get("freshness", 0.5) for v in matching_videos) / len(matching_videos)
            
            # Calculate competition
            channel_diversity = len(set(v["channel_id"] for v in matching_videos)) / len(matching_videos)
            ...
            top_channels = []
            for ch_id, count in sorted(channel_counts.items(), key=lambda x: x[1], reverse=True)[:3]:  # Increased from 2 to 3
            ...
            niche_data = {
                ...
                "sample_videos": [
                    ...
                    for v in sorted(matching_videos, key=lambda x: x["view_count"], reverse=True)[:3]
                ]
            }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--category", default="kids", help="Content category (e.g., kids, tech, finance, health)")
    parser.add_argument("--subcategory", default="kids.nursery", help="Content subcategory (e.g., kids.nursery, tech.programming)")
    parser.add_argument("--api-key", help="YouTube API Key (optional, will use env variable if not provided)")
