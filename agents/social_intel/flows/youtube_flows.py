...
async def store_niche_vectors(niches_df: pd.DataFrame, vector_storage: YouTubeVectorStorage, sentence_transformer):
...
async def get_top_video_for_niche(niche: str) -> str:
    ...
    sorted_videos = sorted(
        videos,
        key=lambda v: int(v.get('viewCount', {}).get('text', '0').replace(',', '')) 
        if isinstance(v.get('viewCount'), dict) else 0,
        reverse=True
    )
...
async def perform_gap_analysis(channels_df: pd.DataFrame, seed_data: Dict[str, Any]) -> pd.DataFrame:
...
def generate_blueprint(
    ...
    for cluster_id, keywords in content_pillars.items():
        pillar_name = f"{keywords[0].title()} & {keywords[1].title()}" if len(keywords) > 1 else keywords[0].title()
...
async def youtube_niche_scout_flow(
    ...
    if not queries:
        queries = [
            "nursery rhymes", "diy woodworking", "urban gardening", 
            "ai news", "budget travel"
        ]
    ...
    result = NicheScoutResult(
        ...
        trending_niches=[
            ...
            for _, row in trending_niches.iterrows()
        ],
        top_niches=[
            YouTubeNiche(**row.to_dict()) 
